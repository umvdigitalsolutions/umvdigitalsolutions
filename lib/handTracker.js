...
export class HandTracker {
    ...
    processHands(landmarks, labels) {
        ...
        const ballGrab = handInfos.find((hand) => hand.ballGrab);
        ...
        const shapeMode = fistHeld ? "pause" : ballGrab ? "hold" : thumbsUp ? "next" : openPalm ? "wake" : "idle";
        ...
        if (mode === "spin") {
            ...
            if (this.prevHoldGrab) {
                const dx = ballGrab.center.x - this.prevHoldGrab.x;
                const dy = ballGrab.center.y - this.prevHoldGrab.y;
                if (Math.abs(dx) > 1e-4 || Math.abs(dy) > 1e-4) {
                    this.callbacks.onRotate(dx * GRAB_ROTATE_SPEED, dy * GRAB_ROTATE_SPEED);
                }
            }
            this.prevHoldGrab = ballGrab.center;
        }
    ...
    drawOverlay(landmarks) {
        ...
        for (const lm of landmarks) {
            ...
            const handScale = dist2d(lm[WRIST], lm[MIDDLE_MCP]);
            const shape = handScale > 1e-6 ? classifyHandShape(lm, handScale) : { ballGrab: false };
            const activeGrab = shape.ballGrab || (handScale > 1e-6 && dist2d(thumb, index) / handScale < PINCH_ON);
            ctx.strokeStyle = activeGrab ? "#8df8ff" : "rgba(90,242,255,0.5)";
            ctx.lineWidth = activeGrab ? 2 : 1;
            ctx.beginPath();
            ctx.moveTo(tx, ty);
            ctx.lineTo(ix, iy);
            ctx.stroke();
            ctx.fillStyle = activeGrab ? "#8df8ff" : "rgba(90,242,255,0.7)";
            const points = shape.ballGrab
                ? [THUMB_TIP, INDEX_TIP, MIDDLE_TIP, RING_TIP, PINKY_TIP].map((index) => [(1 - lm[index].x) * width, lm[index].y * height])
                : [
                    [tx, ty],
                    [ix, iy],
                ];
            for (const [x, y] of points) {
                ctx.beginPath();
                ctx.arc(x, y, activeGrab ? 5 : 3, 0, Math.PI * 2);
                ctx.fill();
            }
        }
    }
...
function classifyHandShape(lm, handScale) {
    ...
    const thumbIndexGap = dist2d(lm[THUMB_TIP], lm[INDEX_TIP]) / handScale;
    const thumbPinkyGap = dist2d(lm[THUMB_TIP], lm[PINKY_TIP]) / handScale;
    const palmWidth = dist2d(lm[INDEX_MCP], lm[PINKY_MCP]) / handScale;
    const tipSpread = dist2d(lm[INDEX_TIP], lm[PINKY_TIP]) / handScale;
    const fingersArcAroundPalm = [INDEX_TIP, MIDDLE_TIP, RING_TIP, PINKY_TIP].every((tip) => lm[tip].y < lm[WRIST].y - handScale * 0.2);
    const hologramHold = extendedCount >= 4 &&
        thumbIndexGap > 0.34 &&
        thumbPinkyGap > 0.8 &&
        tipSpread > palmWidth * 0.9 &&
        fingersArcAroundPalm;
    const openPalm = extendedCount >= 4 && !hologramHold;
    const curledFingers = [INDEX_TIP, MIDDLE_TIP, RING_TIP, PINKY_TIP].filter((tip) => dist2d(lm[tip], lm[WRIST]) < handScale * 1.62).length;
    const tightCurl = [INDEX_TIP, MIDDLE_TIP, RING_TIP, PINKY_TIP].every((tip) => dist2d(lm[tip], lm[WRIST]) < handScale * 1.34);
    const fist = extendedCount <= 1 && tightCurl;
    const curledBallGrab = !fist && !openPalm && curledFingers >= 3 && thumbIndexGap > 0.36 && thumbPinkyGap > 0.72;
    const ballGrab = hologramHold || curledBallGrab;
