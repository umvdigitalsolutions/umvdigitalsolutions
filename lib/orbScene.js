...
export function createOrbScene(container, options = {}) {
    ...
    controls.enableRotate = !scrollDriven;
    controls.enablePan = false;
    controls.enabled = !scrollDriven;
    if (scrollDriven) {
    ...
    function wake() {
        isPaused = false;
        if (forcedServiceReveal < 0.3)
            forcedServiceReveal = 0.3;
    }
    function setPaused(paused) {
        isPaused = paused === true;
    }
    function showAllServices() {
        forcedServiceReveal = 1;
    }
    function highlightAt(x, y) {
        ...
        const column = Math.floor(clampedX * 4);
        const row = Math.floor(clampedY * 3);
        highlightedServiceIndex = Math.min(services.length - 1, Math.max(0, row * 4 + column));
        highlightUntil = clock.getElapsedTime() + 1.6;
        forcedServiceReveal = Math.max(forcedServiceReveal, getServiceRevealDelay(highlightedServiceIndex) + 0.22);
    }
    function resetView() {
        camera.position.copy(initialPosition);
    ...
    function animate() {
        ...
        const motionScale = isPaused ? 0 : 1;
        forcedServiceReveal = Math.max(0, forcedServiceReveal - delta * 0.22 * motionScale);
        if (highlightUntil && realTime > highlightUntil) {
            highlightedServiceIndex = -1;
            highlightUntil = 0;
        }
        ...
        debris.forEach((d) => {
            ...
            d.rotation.x += 0.015 * motionScale;
            d.rotation.z += 0.01 * motionScale;
        });
        const cameraDistance = camera.position.distanceTo(controls.target);
        const cameraServiceReveal = THREE.MathUtils.clamp((SERVICE_REVEAL_START - cameraDistance) / (SERVICE_REVEAL_START - SERVICE_REVEAL_END), 0, 1);
        const serviceReveal = Math.max(cameraServiceReveal, forcedServiceReveal);
        ...
        serviceTextGroup.rotation.y += (0.0009 + serviceReveal * 0.0006) * motionScale;
        ...
        serviceSprites.forEach((sprite, index) => {
            ...
            u.theta += u.speed * (1 + serviceReveal) * motionScale;
            ...
            const highlighted = u.isPrimary && highlightedServiceIndex >= 0 && index === highlightedServiceIndex;
            const pulse = u.isPrimary ? 0.96 + Math.sin(t * u.pulseSpeed + u.floatPhase) * 0.04 : 0.82 + Math.sin(t * u.pulseSpeed + u.floatPhase) * 0.1;
            sprite.material.opacity = Math.min(1, u.baseOpacity * labelReveal * pulse + (highlighted ? 0.34 : 0));
            const distanceScale = THREE.MathUtils.clamp(cameraDistance / SERVICE_REVEAL_START, u.isPrimary ? 0.72 : 0.58, 1);
            const scaleBase = u.isPrimary ? 0.94 + labelReveal * 0.08 + (highlighted ? 0.18 : 0) : 0.86 + labelReveal * 0.14;
